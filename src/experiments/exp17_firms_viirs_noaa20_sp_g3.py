...
Arquivo `src/experiments/exp17_firms_viirs_noaa20_sp_g3.py` mantem uma parte reproduzivel do pipeline, com contratos explicitos para dados reais, avaliacao temporal e manutencao do projeto."""
...
OUT_DIR = PROJECT_ROOT / "outputs" / "exp17_firms_viirs_noaa20_sp_g3"
TARGET_SNAPSHOT = PROJECT_ROOT / "data" / "snapshots" / "inpe_local_v2" / "inpe_monthly_merged.csv"
FIRMS_MONTHLY = PROJECT_ROOT / "data" / "snapshots" / "firms_viirs_noaa20_sp_ce_v1" / "monthly_firms_features.csv"
FIRMS_MANIFEST = PROJECT_ROOT / "data" / "snapshots" / "firms_viirs_noaa20_sp_ce_v1" / "manifest.json"
...
def sha256_file(path: Path) -> str:
    ...
    A funcao faz parte de `src/experiments/exp17_firms_viirs_noaa20_sp_g3.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    h = hashlib.sha256()
...
def build_predictions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for mode, window, lam in configs:
        name = f"firms_{mode}_roll{window}_l{str(lam).replace('.', 'p')}"
        rows_by_model[name] = []
        notes[name] = (
            "firms_lagged_allocator",
            f"EXP-10 total-preserving allocation blended with lagged FIRMS {mode} share; window={window}, lambda={lam}.",
        )
    ...
    for cut in TEST_MONTHS:
        ...
        for mode, window, lam in configs:
            ...
            pred = total * ((1.0 - lam) * base_share + lam * firms_share)
            out = test.copy()
            out["y_pred"] = pred
...
def main() -> None:
    ...
    L? argumentos, chama as rotinas principais de `src/experiments/exp17_firms_viirs_noaa20_sp_g3.py` e retorna erro claro quando o contrato operacional nao e atendido."""
    ...
    report = {
        "experiment_id": "EXP-2026-07-10-17",
        "run_at": datetime.now(timezone.utc).isoformat(),
        "hypothesis": "Lagged NASA FIRMS VIIRS_NOAA20_SP municipal activity improves EXP-10 allocation without target-month leakage.",
    ...
    print("=== EXP-17 FIRMS VIIRS_NOAA20_SP lagged allocation ===")
