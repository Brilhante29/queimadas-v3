...
Arquivo `src/experiments/g5_conformal_coverage_stratified.py` mantem uma parte reproduzivel do pipeline, com contratos explicitos para dados reais, avaliacao temporal e manutencao do projeto."""
...
OUT_DIR = PROJECT_ROOT / "outputs" / "g5_conformal_coverage_stratified"
...
def run(pred_path: Path = PRED_PATH, champion: str = CHAMPION, out_dir: Path = OUT_DIR) -> dict:
    ...
    A funcao faz parte de `src/experiments/g5_conformal_coverage_stratified.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    ...
    p90_dry = float(calib[calib["is_dry"]]["abs_error"].quantile(0.90))
    p90_wet = float(calib[~calib["is_dry"]]["abs_error"].quantile(0.90))

    test["p90_by_regime"] = test["is_dry"].map({True: p90_dry, False: p90_wet})
    test["interval_low"] = (test["y_pred"] - test["p90_by_regime"]).clip(lower=0.0)
    test["interval_high"] = test["y_pred"] + test["p90_by_regime"]
    ...
    manifest = {
        ...
        "champion": champion,
        "protocol": f"calibração ESTRATIFICADA por regime em {CALIB_YEAR} (12 cortes), teste congelado em {TEST_YEAR} (12 cortes)",
        "p90_abs_error_dry_calibrated": p90_dry,
        "p90_abs_error_wet_calibrated": p90_wet,
        ...
        "gate_G5": gate,
        "comparison_vs_v1_global_band": {
            "v1_overall_coverage": 0.89375,
            "v1_dry_coverage": 0.7181818181818181,
            "v1_gate": "FAIL",
        },
    }
    (out_dir / "g5_report.json").write_text(json.dumps(manifest, indent=2, ensure_ascii=False), encoding="utf-8")

    print("=== G5 v2 — Cobertura estratificada por regime (calibração 2023 -> teste 2024) ===")
    print(f"p90 erro absoluto — seco: {p90_dry:.4f} | demais: {p90_wet:.4f}")
    print(f"Cobertura em 2024 (nunca visto): geral={overall_coverage:.4f} seco={dry_cov:.4f} demais={wet_cov:.4f}")
