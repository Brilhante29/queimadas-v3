...
Arquivo `src/experiments/exp19_firms_regional_total_g3.py` mantem uma parte reproduzivel do pipeline, com contratos explicitos para dados reais, avaliacao temporal e manutencao do projeto."""
...
OUT_DIR = PROJECT_ROOT / "outputs" / "exp19_firms_regional_total_g3"
TARGET_SNAPSHOT = PROJECT_ROOT / "data" / "snapshots" / "inpe_local_v2" / "inpe_monthly_merged.csv"
FIRMS_MONTHLY = PROJECT_ROOT / "data" / "snapshots" / "firms_multi_sensor_ce_v1" / "monthly_firms_features.csv"
FIRMS_MANIFEST = PROJECT_ROOT / "data" / "snapshots" / "firms_multi_sensor_ce_v1" / "manifest.json"
...
WINDOWS = [1, 3, 6, 12]
LAMBDAS = [0.0, 0.10, 0.25]
TOTAL_LAMBDAS = [0.10, 0.25, 0.50]
SCORE_MODES = ["count", "frp", "count_frp", "confidence_weighted"]
...
def sha256_file(path: Path) -> str:
    ...
    A funcao faz parte de `src/experiments/exp19_firms_regional_total_g3.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    h = hashlib.sha256()
...
def add_firms_features(df: pd.DataFrame) -> pd.DataFrame:
    """Executa a etapa `add firms features` do fluxo FireCast.
    
    A funcao faz parte de `src/experiments/exp19_firms_regional_total_g3.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    firms = pd.read_csv(FIRMS_MONTHLY)
    firms = firms[["geocodigo", "ano", "mes", *RAW_FIRMS_COLS]].copy()
    firms["geocodigo"] = firms["geocodigo"].astype(int)
    merged = df.merge(firms, on=["geocodigo", "ano", "mes"], how="left")
    for col in RAW_FIRMS_COLS:
    ...
    for col in RAW_FIRMS_COLS:
        ...
        for window in (3, 6, 12):
            merged[f"{col}_roll{window}"] = by_geo[col].transform(lambda x, w=window: x.shift(1).rolling(w, min_periods=1).mean())
    return merged


def score_from_firms(test: pd.DataFrame, mode: str, window: int) -> np.ndarray:
    """Calcula a etapa `score from firms` do fluxo FireCast.
    
    A funcao faz parte de `src/experiments/exp19_firms_regional_total_g3.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    count_col = f"firms_fire_count_roll{window}" if window != 1 else "firms_fire_count_lag1"
    frp_col = f"firms_frp_sum_roll{window}" if window != 1 else "firms_frp_sum_lag1"
    conf_col = "firms_confidence_mean_lag1"
    count = np.log1p(np.maximum(test[count_col].fillna(0.0).to_numpy(dtype=float), 0.0))
    frp = np.log1p(np.maximum(test[frp_col].fillna(0.0).to_numpy(dtype=float), 0.0))
    conf = np.clip(test[conf_col].fillna(0.0).to_numpy(dtype=float) / 100.0, 0.0, 1.0)
    if mode == "count":
        score = count
    elif mode == "frp":
        score = frp
    elif mode == "count_frp":
        score = 0.55 * count + 0.45 * frp
    elif mode == "confidence_weighted":
        score = (0.55 * count + 0.45 * frp) * (0.5 + conf)
    else:
...
def regional_signal(frame: pd.DataFrame, mode: str, window: int) -> np.ndarray:
    """Executa a etapa `regional signal` do fluxo FireCast.
    
    A funcao faz parte de `src/experiments/exp19_firms_regional_total_g3.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    return score_from_firms(frame, mode, window)


def expected_regional_signal(train: pd.DataFrame, eligible: pd.Index, cut: pd.Period, mode: str, window: int) -> float:
    """Executa a etapa `expected regional signal` do fluxo FireCast.
    
    A funcao faz parte de `src/experiments/exp19_firms_regional_total_g3.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    hist = train[(train["geocodigo"].isin(eligible)) & (train["mes"] == cut.month)].copy()
    if hist.empty:
        return 0.0
    hist["_firms_signal"] = regional_signal(hist, mode, window)
    period_signal = hist.groupby("period")["_firms_signal"].sum()
    positive = period_signal[period_signal > 0]
    if len(positive) > 0:
        return float(positive.median())
    return float(period_signal.mean()) if len(period_signal) else 0.0
...
def build_predictions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    configs = [
        (mode, window, total_lam, alloc_lam)
        for mode in SCORE_MODES
        for window in WINDOWS
        for total_lam in TOTAL_LAMBDAS
        for alloc_lam in LAMBDAS
    ]
    for mode, window, total_lam, alloc_lam in configs:
        name = (
            f"firms_total_{mode}_roll{window}_tl{str(total_lam).replace('.', 'p')}"
            f"_al{str(alloc_lam).replace('.', 'p')}"
        )
        rows_by_model[name] = []
        notes[name] = (
            "firms_regional_total_allocator",
            "EXP-10 prediction scaled by lagged FIRMS regional activity against past same-month FIRMS signal, "
            f"then optionally blended with FIRMS allocation; mode={mode}, window={window}, total_lambda={total_lam}, alloc_lambda={alloc_lam}.",
        )
    ...
    for cut in TEST_MONTHS:
        ...
        for mode, window, total_lam, alloc_lam in configs:
            ...
            firms_score = score_from_firms(test, mode, window)
            firms_share = normalized_share(firms_score, base_pred)
            current_signal = float(regional_signal(test, mode, window).sum())
            expected_signal = expected_regional_signal(train, eligible, cut, mode, window)
            signal_ratio = (current_signal + 1.0) / (expected_signal + 1.0) if expected_signal >= 0.0 else 1.0
            signal_ratio = float(np.clip(signal_ratio, 0.50, 2.00))
            total_scale = (1.0 - total_lam) + total_lam * signal_ratio
            pred = total * total_scale * ((1.0 - alloc_lam) * base_share + alloc_lam * firms_share)
            out = test.copy()
            out["y_pred"] = pred
...
def main() -> None:
    ...
    L? argumentos, chama as rotinas principais de `src/experiments/exp19_firms_regional_total_g3.py` e retorna erro claro quando o contrato operacional nao e atendido."""
    ...
    report = {
        "experiment_id": "EXP-2026-07-10-19",
        "run_at": datetime.now(timezone.utc).isoformat(),
        "hypothesis": "Lagged NASA FIRMS multi-sensor activity improves EXP-10 regional total and allocation without target-month leakage.",
        ...
        "firms_manifest_sha256": sha256_file(FIRMS_MANIFEST),
        "firms_monthly_sha256": sha256_file(FIRMS_MONTHLY),
    ...
    print("=== EXP-19 FIRMS multi-sensor regional-total correction ===")
