...
Arquivo `src/experiments/exp15_firms_modis_sp_g3.py` mantem uma parte reproduzivel do pipeline, com contratos explicitos para dados reais, avaliacao temporal e manutencao do projeto."""
...
OUT_DIR = PROJECT_ROOT / "outputs" / "exp15_firms_modis_sp_g3"
TARGET_SNAPSHOT = PROJECT_ROOT / "data" / "snapshots" / "inpe_local_v2" / "inpe_monthly_merged.csv"
FIRMS_MONTHLY = PROJECT_ROOT / "data" / "snapshots" / "firms_modis_sp_ce_v1" / "monthly_firms_features.csv"
FIRMS_MANIFEST = PROJECT_ROOT / "data" / "snapshots" / "firms_modis_sp_ce_v1" / "manifest.json"
...
def sha256_file(path: Path) -> str:
    ...
    A funcao faz parte de `src/experiments/exp15_firms_modis_sp_g3.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    h = hashlib.sha256()
...
def main() -> None:
    ...
    L? argumentos, chama as rotinas principais de `src/experiments/exp15_firms_modis_sp_g3.py` e retorna erro claro quando o contrato operacional nao e atendido."""
    ...
    report = {
        "experiment_id": "EXP-2026-07-10-15",
        "run_at": datetime.now(timezone.utc).isoformat(),
        "hypothesis": "Lagged NASA FIRMS MODIS_SP municipal activity improves EXP-10 allocation without target-month leakage.",
    ...
    print("=== EXP-15 FIRMS MODIS_SP lagged allocation ===")
