...
Arquivo `src/experiments/exp18_firms_multi_sensor_g3.py` mantem uma parte reproduzivel do pipeline, com contratos explicitos para dados reais, avaliacao temporal e manutencao do projeto."""
...
OUT_DIR = PROJECT_ROOT / "outputs" / "exp18_firms_multi_sensor_g3"
...
def sha256_file(path: Path) -> str:
    ...
    A funcao faz parte de `src/experiments/exp18_firms_multi_sensor_g3.py` e deve preservar rastreabilidade, determinismo e separacao entre treino, avaliacao e serving."""
    h = hashlib.sha256()
...
def main() -> None:
    ...
    L? argumentos, chama as rotinas principais de `src/experiments/exp18_firms_multi_sensor_g3.py` e retorna erro claro quando o contrato operacional nao e atendido."""
    ...
    report = {
        "experiment_id": "EXP-2026-07-10-18",
        "run_at": datetime.now(timezone.utc).isoformat(),
        "hypothesis": "Lagged NASA FIRMS multi-sensor municipal activity improves EXP-10 allocation without target-month leakage.",
    ...
    print("=== EXP-18 FIRMS multi-sensor lagged allocation ===")
